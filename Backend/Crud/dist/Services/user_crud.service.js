import { Mongo_User } from "../Config/user_db.js";
import logger from "./logs.service.js";
import { Encriptacion, Desencriptar, } from "./encriptar_desencriptar.service.js";
export class User_Crud {
     ...
     */
    async Create(Name, Password, Phone, Avatar) {
        try {
     ...
     * @returns Estado y mensaje del proceso
     */
    async Update(Id, Name, Password, Phone, Avatar) {
        try {
            const Phone_Encriptado = await Encriptacion(Phone);
            const Password_Encriptado = await Encriptacion(Password);
            if (!Phone_Encriptado.state) {
            ...
            const process = await Mongo_User.findByIdAndUpdate(Id, {
                Name,
                Password: Password_Encriptado.content,
                Phone: Phone_Encriptado.content,
                Avatar,
     ...
     * @returns Estado y mensaje del proceso
     */
    async Read_Credenciales(Password, Phone) {
        try {
            const Phone_Encriptado = await Encriptacion(Phone);
            const Password_Encriptado = await Encriptacion(Password);
            if (!Phone_Encriptado.state) {
